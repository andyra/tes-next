  ...
  useEffect(() => {
    ...
    for (let u = 0; u < mapSize; u++) {
      for (let v = 0; v < mapSize; v++) {
        ...
        // TODO: Higher stretch values = more ripples. Change the first # (3)
        // stretching so we get the desired ripple density on our map
        const stretch = (shapeA * Math.PI) / (mapSize / 2);
    ...
    for (let u = 0; u < mapSize; u++) {
      for (let v = 0; v < mapSize; v++) {
        ...
        const d2 = distance(1.35 * cx, shapeB * cy) * 0.022;
    ...
    // TODO: Change the 3 here; lower is faster
    const tick = time => {
      moveHeightMaps(time / mySpeed);
      updatePalette(time / mySpeed);
      updateImageData();
