...
export const Queue = ({ isFullscreen, playerIsEmpty, setIsFullscreen }) => {
  ...
  const contentClasses = cn({
    ...
    "flex flex-col bg-ground md:rounded-xl": true,
  ...
  return (
    <Dialog.Root modal={false}>
      ...
      <Dialog.Portal>
        {/*<Dialog.Overlay className="fixed top-0 right-0 bottom-0 left-0 backdrop-blur-md bg-primary-5 radix-state-open:animate-fade-in z-dialog" />*/}
          ...
          <div id="queue-content" className={contentClasses}>
            <Dialog.Close asChild>
              <Button
                circle
                iconLeft="X"
                className="absolute top-16 right-16 z-dialog-content"
              />
            </Dialog.Close>
            <div className="flex-1 p-24 overflow-y-scroll">
              {currentTrack && (
            ...
            <footer className="sticky bottom-0 z-10 bg-ground p-24 border flex items-center justify-center gap-4 md:hidden">
              <div className="h-48 w-48 rounded-full bg-primary-10" />
              <div className="h-48 w-48 rounded-full bg-primary-10" />
              <div className="h-48 w-48 rounded-full bg-primary-10" />
              <div className="h-48 w-48 rounded-full bg-primary-10" />
              <div className="h-48 w-48 rounded-full bg-primary-10" />
